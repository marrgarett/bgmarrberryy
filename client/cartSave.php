...
if (isset($_GET["bgmarr_name"])) {
    ...
    $sql = "INSERT INTO `id_order` (`id`, `id_order`, `session_user_id`, `user_id`,  `id_bgmarr_name`, `price`, `quantity_hr`, `date_order`, `status`) 
        ...
        if ($type == "image/jpg" || $type == 'image/jpeg' || $type == "image/png" || $type == "image/gif") {
            if (!file_exists($path)) { // check file not exist in your upload folder path
                if ($size < 15000000) { // check file size 15MB
                    ...
                    $stmt = $db->prepare("UPDATE `history_tbl` SET  his_payment = :img_name
                    WHERE `history_tbl`.`order_id` = '$id_order'");
                    $stmt->bindParam(':img_name', $name);
                    if ($stmt->execute()) {
                        $sql = "UPDATE `id_order` SET `status` = 'Complete'
